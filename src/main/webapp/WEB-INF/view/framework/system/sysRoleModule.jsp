  ...
  <body>
  	...
  	<div id="divLayout" class="easyui-layout" style="width:auto;height:450px">
        <div id="divCenter" data-options="region:'center'">
		    <table id="dataList" title="角色模块列表" class="easyui-datagrid" style="width:auto;height:auto"
		    		url="${ctx}/sysRoleModule/getListData.do"
...
模块主键<input id="sel_moduleid" name="sel_moduleid" class="easyui-textbox" data-options=""/>
    ...
    <script type="text/javascript">
        ...
        function newInfo(){
            ...
            url = '${ctx}/sysRoleModule/saveByAdd.do';
        }
        function editInfo(){
            ...
            if (row){
                ...
                url = '${ctx}/sysRoleModule/saveByUpdate.do?id='+row.id;
            }    	
        }
        var loadi;
        function saveInfo(){
            ...
            if (row){
                $.messager.confirm('确认','你确定删除该记录吗？',function(r){
                    if (r){
						...
						$('#delForm').form('submit',{
							url: '${ctx}/sysRoleModule/delete.do',
							dataType: 'text',
							success: function(info){
								if (info==TRUE){
			                        layer.msg("操作成功！", 2, -1);
