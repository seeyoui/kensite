...
1. 平台代码迁移：公司各项目之间进行模块迁移，比如露营地供应链模块，通过该工具可以生成一个plugs，放到公司别的项目里可以直接使用。
2. 统计图表设计器：基于Echarts开发可视化自定义设计器，通过设计器可以快速配置出各种常用统计图，如折线图，柱状图，饼图，雷达图等。
3. 统计报表设计器：基于spreadjs开发可视化自定义配置工具，通过配置可以快速生成各种统计报表，如各类财务报表等。
4. 规则引擎集成：
5. 规则引擎设计器：
6. 流程引擎集成：
7. 流程表单设计器：
